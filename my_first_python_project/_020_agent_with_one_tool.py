"""
_020_agent_with_one_tool.py

The exact same Agent class from File _019, with one addition: a `tools`
dict, and the validated currency tool from File _018 dropped into it.
The agent can now hold both its brain AND a working tool -- it still
doesn't decide on its own when to use it. That's Class 4.

Run with: uv run _020_agent_with_one_tool.py
"""
...
# --- The Agent, now with a `tools` attribute (File _019 + one new field) ---
...
if __name__ == "__main__":
    ...
    print(trip_planner.ask_brain("How about travel?"))
